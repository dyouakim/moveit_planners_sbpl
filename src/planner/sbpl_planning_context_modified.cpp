...
bool SBPLPlanningContext::initSBPL(std::string& why)
{
    ...
    scene_nonconst->getPlanningSceneMsg(sceneMsg);
    std::vector<moveit_msgs::CollisionObject> collisionObjects = sceneMsg.world.collision_objects;
    ROS_ERROR_STREAM("SBPL CONTEXT Number of collision objects "<<collisionObjects.size());
    ...
    const moveit_msgs::WorkspaceParameters& workspace)
{
    ...
    ROS_INFO_NAMED(PP_LOGGER, "  origin_x: %0.3f", workspace_pos_in_planning.x());
    ROS_INFO_NAMED(PP_LOGGER, "  origin_y: %0.3f", workspace_pos_in_planning.y());
    ROS_INFO_NAMED(PP_LOGGER, "  origin_z: %0.3f", workspace_pos_in_planning.z());
    ...
    if (sbpl_cworld) {
        ROS_ERROR_STREAM("Use collision information from Collision World SBPL for heuristic!!!");
        ...
        if (df) {
            ...
            ROS_ERROR_STREAM("Copy collision information");
